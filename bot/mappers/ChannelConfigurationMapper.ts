...
export class ChannelConfigurationMapper {
    ...
    static toModel(domain: Omit<ChannelConfiguration, "id">): Omit<ChannelConfigurationModel, "id"> {
        return {
    ...
    static toPartialModel(domain: Partial<Omit<ChannelConfiguration, "id">>): Partial<Omit<ChannelConfigurationModel, "id">> {
        return {
            channelId: domain.channelId,
            tiers: domain.tiers?.map(TierMapper.toModel),
            preferredLanguage: domain.preferredLanguage ? LanguageMapper.toModel(domain.preferredLanguage) : undefined,
            timezone: domain.timezone ? TimezoneMapper.toModel(domain.timezone) : undefined,
            dailyNotificationTime: null,
        };
    }
}