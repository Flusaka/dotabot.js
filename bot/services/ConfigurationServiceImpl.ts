...
import { AddTierResult, RemoveTierResult, SetDailyNotificationTimeResult, SetNotificationTimezoneResult, SetPreferredLanguageResult, type ConfigurationService } from "./interfaces/ConfigurationService";
...
export class ConfigurationServiceImpl implements ConfigurationService {
    ...
    async setPreferredLanguage(channelId: bigint, language: Language): Promise<SetPreferredLanguageResult> {
        const channel = await this.channelConfigRepo.getByChannelId(channelId);
        if(!channel) { 
    ...
    async setNotificationTimezone(channelId: bigint, timezone: Timezone): Promise<SetNotificationTimezoneResult> {
        const channel = await this.channelConfigRepo.getByChannelId(channelId);
        if(!channel) { 
    ...
    async setDailyNotificationTime(channelId: bigint, timeString: string): Promise<SetDailyNotificationTimeResult> {
        const channel = await this.channelConfigRepo.getByChannelId(channelId);
        if(!channel) { 
