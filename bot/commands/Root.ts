...
import { ConnectionResult, DisconnectionResult, type ConnectionService } from "../services/interfaces/ConnectionService";
...
class Root {
    ...
    @Slash({description: "Disconnect this channel from DotaBot", name: "disconnect"})
    async disconnect(interaction: CommandInteraction) {
        ...
        switch(result) {
            case DisconnectionResult.Success: {
                await interaction.editReply(`DotaBot successfully disconnected from ${channelMention(interaction.channelId)}!`);
                break;
            }
            case DisconnectionResult.ChannelNotConnected: {
                await interaction.editReply(`DotaBot is not currently connected to ${channelMention(interaction.channelId)}, no need to disconnect!`);
                break;
            }
            default: {
                await interaction.editReply(`DotaBot couldn't be disconnected from ${channelMention(interaction.channelId)} at the moment :( Please try again later!`);
                break;
            }
        }
    }
