...
describe('ChannelConfiguration tests', () => {
    test('addTier should push to tiers array and return true, if tier is not currently in configuration', () => {
        // 1. Arrange
        const config = ChannelConfiguration.fromExisting(0, 0n, [Tier.S], Timezone.GMT, Language.English);
    ...
    test('removeTier should remove from tiers array and return true, if tier is currently in configuration', () => {
        // 1. Arrange
        const config = ChannelConfiguration.fromExisting(0, 0n, [Tier.S, Tier.A], Timezone.GMT, Language.English);
    ...
    ])('setDailyNotificationTime should set dailyNotificationTime to correct value, if timeString is valid format', (timeString, time) => {
        // 1. Arrange
        const config = ChannelConfiguration.fromExisting(0, 0n, [Tier.S], Timezone.GMT, Language.English);
    ...
    test.each([
        ...
        "",
        "[][as9"
    ])('setDailyNotificationTime throws TimeParseError, if timeString is invalid format', (timeString) => {
        // 1. Arrange
        const config = ChannelConfiguration.fromExisting(0, 0n, [Tier.S], Timezone.GMT, Language.English);

        // 2. Act
        // 3. Assert
        expect(() => config.setDailyNotificationTime(timeString)).toThrowError(TimeParseError);
    });
});